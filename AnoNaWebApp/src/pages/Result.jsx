// pages/Result.jsx - Updated to show added choices
import { useEffect, useState } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { supabase } from '../lib/supabase';
import { Share2, Trophy, RefreshCw, Crown, Target, PlusCircle } from 'lucide-react';
...
export default function Result() {
  ...
  useEffect(() => {
    ...
    confetti({
      ...
      spread: 70,
      origin: { y: 0.6 },
      colors: ['#1B4D3E', '#2D6A4F', '#52B788', '#95D5B2']
    });
    setTimeout(() => {
      confetti({
        particleCount: 40,
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-b from-white to-[#F5FEF7]">
      <div className="px-5 py-12 max-w-lg mx-auto pb-32">
        {/* Yes or No Illustration for tie or no votes */}
        {(total === 0 || isTie) && (
          <Illustration type="yesNo" size="sm" className="mb-4" animate={false} />
        )}
        ...
        >
          ...
          <motion.div 
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-[#E9F5E8] mb-5"
          >
            {icon}
          </motion.div>
          ...
          <motion.h1 
            ...
            className={`text-5xl font-black mb-2 ${
              total === 0 ? 'text-[#84A98C]' : isTie ? 'text-[#52B788]' : 'bg-gradient-to-r from-[#1B4D3E] to-[#52B788] bg-clip-text text-transparent'
            }`}
          >
            {status}
          </motion.h1>
          ...
          <button
            onClick={shareResult}
            className="w-full py-3 bg-gradient-to-r from-[#1B4D3E] to-[#2D6A4F] text-white rounded-xl flex items-center justify-center gap-2 text-sm font-semibold shadow-md hover:shadow-lg transition-all"
          >
      ...
      <FloatingBackButton onClick={() => navigate('/')} label="Create New Poll" />
    </div>
  );
}