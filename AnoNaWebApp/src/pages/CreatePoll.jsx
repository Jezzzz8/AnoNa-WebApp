...
import { Plus, Clock, Send, Sparkles, Users, PlusCircle } from 'lucide-react';
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          ...
          <div className="flex items-center gap-4 mb-3">
            <div className="w-16 h-16 rounded-2xl bg-[#52B788]/10 flex items-center justify-center">
              <Sparkles size={32} className="text-[#52B788]" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-[#1B4D3E]">Create a Poll</h1>
          ...
          <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.15 }}>
            <label className="block text-sm font-semibold text-[#52796F] mb-2">Question</label>
            <div className={`relative transition-all duration-300 ${focusedField === 'question' ? 'scale-[1.01]' : ''}`}>
              ...
              />
              <div className="absolute bottom-3 right-3 text-xs text-[#84A98C]">{questionCharCount}/200</div>
            </div>
          </motion.div>

          {/* Options Grid */}
          <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.2 }}>
            <label className="block text-sm font-semibold text-[#52796F] mb-2">Choices</label>
          ...
          {/* Allow Add Choice Toggle */}
          <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.23 }}>
            <div className={`rounded-xl p-4 transition-all duration-300 ${allowAddChoice ? 'bg-[#52B788]/5 border border-[#52B788]/30' : 'bg-[#E9F5E8]/50 border border-[#D8F3DC]'}`}>
              <div className="flex items-center justify-between">
                ...
                <Toggle checked={allowAddChoice} onChange={setAllowAddChoice} size="md" />
              </div>
              <AnimatePresence>
                {allowAddChoice && (
                  <motion.div
          ...
          <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.25 }}>
            ...
            <div className="grid grid-cols-4 gap-2 mb-3">
              {timerPresets.map(({ sec, label, icon }) => (
                <motion.button
                  ...
                  className={`py-2 rounded-lg text-sm font-medium transition-all ${timer === sec ? 'bg-[#2D6A4F] text-white' : 'bg-white border border-[#D8F3DC] text-[#2D6A4F] hover:border-[#52B788]'}`}
                >
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 bg-white border border-[#D8F3DC] rounded-lg text-center text-[#1B4D3E] text-sm placeholder:text-[#84A98C] focus:border-[#52B788] focus:ring-2 focus:ring-[#52B788]/20"
              />
              <button onClick={setCustomTimer} className="px-4 py-2 bg-[#E9F5E8] rounded-lg text-[#2D6A4F] text-sm font-medium hover:bg-[#D8F3DC]">
                Apply
              </button>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }}>
            <button
              ...
              disabled={isCreating}
              className="w-full py-4 bg-gradient-to-r from-[#1B4D3E] to-[#2D6A4F] text-black rounded-xl text-base font-semibold flex items-center justify-center gap-2 transition-all disabled:opacity-70"
            >
              {isCreating ? (
                <>
